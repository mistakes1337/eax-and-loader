...
void Visuals::DrawPlayer( Player* player ) {
	...
	if (!player->dormant()) {
		...
		valid_dormant = g_sound.AdjustPlayerBegin(player); // something in here is fucking up the fading animations sometimes(goes up the air when enemy dies)
	...
	if (name_esp) {
		...
		*/
		...
		if (g_menu.main.players.name_style.get() == 0) {
			if (dormant)
				render::logevent.string(box.x + box.w / 2, box.y - render::logevent.m_size.m_height, Color(210, 210, 210, low_alpha), name, render::ALIGN_CENTER);
			else
				render::logevent.string(box.x + box.w / 2, box.y - render::logevent.m_size.m_height, Color(clr.r(), clr.g(), clr.b(), g_menu.main.players.name_esp_alpha.get()).alpha(low_alpha), name, render::ALIGN_CENTER);
		}
		else {
			if (dormant)
				render::esp_other.string(box.x + box.w / 2, box.y - render::esp_other.m_size.m_height, Color(210, 210, 210, low_alpha), name, render::ALIGN_CENTER);
			else
				render::esp_other.string(box.x + box.w / 2, box.y - render::esp_other.m_size.m_height, Color(clr.r(), clr.g(), clr.b(), g_menu.main.players.name_esp_alpha.get()).alpha(low_alpha), name, render::ALIGN_CENTER);
		}
	...
	if( health_esp ) {
		...
		if (player->m_iHealth() <= 92) {
			render::esp_small.string(box.x - 5, y + (h - fill) - 5, dormant ? Color(220, 220, 220, weapon_alpha) : Color(255, 255, 255, weapon_alpha), std::to_string(hp), render::ALIGN_CENTER);
		}
	}
	...
	// draw flags.
	{
		...
		for( auto it = items.begin( ); it != items.end( ); ++it ) {
			...
			if( *it == 3 && player->m_flFlashBangTime( ) > 0.f )
				if (dormant)
				flags.push_back( { XOR( "NO SEE" ), { 210, 210, 210, low_alpha } } );
				else
				flags.push_back({ XOR("NO SEE"), {  0, 175, 255, low_alpha } });
			...
			if( *it == 5 && player->HasC4( ) )
				if (dormant) 
				flags.push_back( { XOR( "BOMB" ), { 210, 210, 210, low_alpha } } );
				else
			...
			if( weapon ) {
				...
				if( data ) {
					...
					if (g_menu.main.players.weaponicon.get()) {
						...
						if (dormant)
							render::cs.string(box.x + (box.w / 2), box.y + box.h + offset, Color(210, 210, 210, low_alpha), GetWeaponIcon(weapon->m_iItemDefinitionIndex()), render::ALIGN_CENTER);
						else
