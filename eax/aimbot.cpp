...
void AimPlayer::pre_anim_update(LagRecord* record, LagRecord* previous) {
	...
	if (previous) {
		...
		anim_state->m_strafe_change_weight = record->m_layers[7].m_weight;
		anim_state->m_strafe_sequence = record->m_layers[7].m_sequence;
		anim_state->m_strafe_change_cycle = record->m_layers[7].m_cycle;
		anim_state->m_acceleration_weight = record->m_layers[12].m_weight;

		// set animstate element to previously networked values
		anim_state->m_abs_yaw = previous->m_foot_yaw;
		anim_state->m_move_yaw = previous->m_move_yaw;
		anim_state->m_move_yaw_current_to_ideal = previous->m_move_yaw_cur_to_ideal;
...
void AimPlayer::post_anim_update(LagRecord* record) {
	...
	record->m_foot_yaw = anim_state->m_abs_yaw;
	record->m_move_yaw = anim_state->m_move_yaw;
	record->m_move_yaw_cur_to_ideal = anim_state->m_move_yaw_current_to_ideal;
	...
	record->m_abs_ang = ang_t(0.f, anim_state->m_abs_yaw, 0.f);
}
...
void Aimbot::think() {
	...
	const auto bones = static_cast<BoneArray*>(g_csgo.m_mem_alloc->Alloc(sizeof(BoneArray) * 128));
...
bool Aimbot::get_aim_matrix(ang_t angle, BoneArray* bones) {
	/* get animstate */
	...
	/* vars */
	float flBackupPoses[24];
	C_AnimationLayer varBackupLayers[15];

	/* get animlayer and pose param */
	g_cl.m_local->GetAnimLayers(varBackupLayers);
	g_cl.m_local->GetPoseParameters(flBackupPoses);

	/* set animlayer and poseparam */
	g_cl.m_local->SetAnimLayers(g_cl.m_layers);
	g_cl.m_local->SetPoseParameters(g_cl.m_poses);
	...
	/* set pose parameter */
	g_cl.m_local->m_flPoseParameter()[12] = flPitch; /* should be fine like that */
	g_cl.m_local->SetAbsAngles(ang_t(0.f, g_cl.m_abs_yaw, 0.f));

	/* setup local bones */
	const auto rSetupMatrix = g_bone_handler.SetupBonesRebuild(g_cl.m_local, bones, 128, BONE_USED_BY_ANYTHING & ~BONE_USED_BY_BONE_MERGE,
		g_cl.m_local->m_flSimulationTime(), 2);

	g_cl.m_local->SetAnimLayers(varBackupLayers);
	g_cl.m_local->SetPoseParameters(flBackupPoses);

	/* return setup bones */
	return rSetupMatrix;
}

void Aimbot::find(BoneArray* bones) {
	...
	if (found_target) {
		...
		if (const auto bone_cache = &g_cl.m_local->m_BoneCache())
		{
			BoneArray* backup_cache = bone_cache->m_pCachedBones;
			const int backup_bone_count = bone_cache->m_CachedBoneCount;
			bone_cache->m_CachedBoneCount = 128;
			bone_cache->m_pCachedBones = bones;
			vec3_t eye = g_cl.m_local->m_vecViewOffset() + g_cl.m_local->GetAbsOrigin();
			for (auto f = 0; f < 10; f++)
			{
				auto look = math::look(g_cl.m_shoot_pos, best.pos);
				if (get_aim_matrix(look - g_cl.m_local->m_aimPunchAngle() * 2, bones))
				{
					//g.m_local->get_eye_pos(&eye);
					g_cl.ModifyEyePosition(g_cl.m_local->m_PlayerAnimState(), bones, &eye);
					if (fabsf(g_cl.m_shoot_pos.z - eye.z) <= 0.01f)
						break;
				}
			}
		...
		ang_t whereshootretard = ang_t();
		math::VectorAngles(best.pos - g_cl.m_shoot_pos, whereshootretard);
		...
		bool hit = on && CheckHitchance(m_target, m_hitbox, whereshootretard);
...
void Aimbot::apply() {
	...
	// if this dont hit just recalc again and loose even more fps )))
	auto look = math::look(g_cl.m_shoot_pos, m_aim);
