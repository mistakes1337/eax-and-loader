...
public:
	...
	animstate_pose_param_cache_t m_pose_param_mappings[COUNT] = {};
	...
	void set_layer_seq(C_AnimationLayer* layer, int act, int overrideseq = -1);
}; // size: 0x344
...
public:
	...
	__forceinline vec3_t WeaponShootPosition() {
	...
	__forceinline void ModifyEyePosition(CCSGOPlayerAnimState* state, vec3_t& pos, matrix3x4_t* matrix) {
		...
		if (state->m_player && (state->m_landing || state->m_player->m_flDuckAmount() != 0.f || !state->m_player->GetGroundEntity())) {
			auto v5 = 8;
