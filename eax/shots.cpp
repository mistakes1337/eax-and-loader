...
void Shots::OnShotFire(Player* target, float damage, int bullets, LagRecord* record, vec3_t aim_point, int hitbox, int hitgroup) {
	...
	if (target && record) {
		...
		std::transform(std::execution::par, 
			shot.m_record->m_resolver_mode.begin(), 
			shot.m_record->m_resolver_mode.end(), 
			shot.m_record->m_resolver_mode.begin(), 
		::tolower);
		...
		if (g_cl.m_userr == "dev") {
			if (success)
				...
				g_notify.add(tfm::format("[dbg] fired shot | ent: %s, hb: %s (%i dmg, %s, %i), mode: %s, bt: %i, vel: %i (%i, [%s:%s]), lag: %s, ext: %s | %s\n",
					info.m_name, 
					m_groups[shot.m_hitgroup], 
					shot.m_damage, 
					data->m_prefer_body,
					hc, 
					shot.m_record->m_resolver_mode,
